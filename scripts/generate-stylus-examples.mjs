 ...
 *
 * Unlike `generate-cli-reference.mjs` this pins no upstream ref. Nitro has releases and
...
import {
  ...
  sections,
  sourceRoot,
} from './data/stylus-examples.data.mjs';
...
import { buildPage, buildSectionMeta } from './lib/stylus-examples.mjs';
...
function parseArgs(argv) {
  const args = { check: isCheckMode(), sourcePath: process.env.STYLUS_REPO_PATH ?? null };
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '--source-path' && argv[i + 1]) args.sourcePath = argv[++i];
  }
  return args;
}
 ...
 * `generate-cli-reference.mjs` applies to Nitro.
 */
function materializeSource({ sourcePath, workDir }) {
  ...
  if (!fs.existsSync(appDir)) {
    throw new Error(
      `${path.join(sourcePath ?? repoUrl, sourceRoot)} does not exist; the upstream repository ` +
        `has been restructured and scripts/data/stylus-examples.data.mjs needs sourceRoot updated`,
    );
  }
  return appDir;
}
 ...
 */
function reportUnpublished(appDir) {
 ...
 */
async function emit(filePath, content, options) {
  try {
        ...
        if (!fs.existsSync(sourceFile)) {
          throw new Error(
            `${context} is gone from upstream. Either it was renamed, in which case update ` +
              `scripts/data/stylus-examples.data.mjs and add a redirect, or it was deleted, in ` +
