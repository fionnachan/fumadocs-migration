...
const SCRIPT = fileURLToPath(new URL('./check-nitro-release.mjs', import.meta.url));
...
function run(
  t,
  { latest = 'v3.11.4', sha = NEW_SHA, status = 200, metadata = {}, vars = VARS, images } = {},
) {
  ...
  return {
    ...
    requests: fs.readFileSync(requestsPath, 'utf8').trim().split('\n').map(JSON.parse),
...
test('a release bump updates the submodule, image and opted-in commands together', (t) => {
  ...
  assert.ok(result.page.includes(result.vars.latestNitroNodeImage));
