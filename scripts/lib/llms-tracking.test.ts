...
test('pathInfo: a .md outside /docs is ignored (the redirect destination is tracked instead)', () => {
  ...
  assert.deepEqual(pathInfo('/anytrust.md', ''), ignored);
});
...
test('pathInfo: the mirror route for the docs index is /docs.md, not /docs/.md', () => {
  assert.deepEqual(pathInfo('/llms.mdx/docs/content.md', ''), {
    kind: 'markdown-mirror',
    trackedPath: '/docs.md',
    fileType: 'page',
  });
...
test('pathInfo: all three request shapes for one page agree on the tracked path', () => {
  const direct = pathInfo('/docs/stylus/quickstart.md', '');
  const mirror = pathInfo('/llms.mdx/docs/stylus/quickstart/content.md', '');
  const negotiated = pathInfo('/docs/stylus/quickstart', 'text/markdown');
...
test('pathInfo: a clean /docs URL with Accept text/markdown is a negotiation', () => {
  assert.deepEqual(pathInfo('/docs/get-started', 'text/markdown'), {
...
test('pathInfo: negotiating the docs index gives /docs.md', () => {
  assert.deepEqual(pathInfo('/docs', 'text/markdown'), {
    kind: 'markdown-negotiate',
    trackedPath: '/docs.md',
    fileType: 'page',
  });
