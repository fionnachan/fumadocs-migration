 ...
 * enforces it during a build was the copy with no tests: `pnpm build` runs with
 * `--experimental-build-mode=compile` and `generateStaticParams` returns `[]` (see CLAUDE.md,
 * "Known trade-off"), so no page or layout module is ever evaluated at build time and the helper's
 * throw never fires there. `next.config.mjs` is the earliest thing a build does evaluate, which is
 * what makes it the enforcement point. One module imported by both means the enforcing copy and the
 * tested copy are the same copy. `tsconfig.json` sets `allowJs`, so `lib/shared.ts` imports this
 * with inferred types and no `.d.ts`.
 *
