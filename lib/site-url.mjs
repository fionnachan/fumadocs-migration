 ...
 * build evaluates, which is what makes it the enforcement point that always fires. Until FS-2689 it
 * was the *only* one that could: `pnpm build` ran with `--experimental-build-mode=compile`, so no
 * page or layout module was evaluated at build time at all and the helper's throw never fired
 * there. That flag is gone and 703 routes now prerender, so `app/layout.tsx`'s module scope runs at
 * build and would throw as well. The config-level check still earns its keep, because the docs
 * route itself stays dynamic (see CLAUDE.md, "Known trade-off") and a build whose prerendered
 * routes happen not to reach `getSiteUrl()` has nothing else to fail it. One module imported by
 * both means the enforcing copy and the tested copy are the same copy. `tsconfig.json` sets
 * `allowJs`, so `lib/shared.ts` imports this with inferred types and no `.d.ts`.
 *
