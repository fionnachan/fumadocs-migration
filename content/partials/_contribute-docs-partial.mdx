...
{/* Every github.com URL in this file hardcodes the owner, repo and branch that gitConfig in lib/shared.ts owns. The Var component cannot sit inside a markdown link destination (CommonMark tokenises the destination before MDX evaluates it), so retarget all of them by hand if the repo or default branch moves. The Banner conventions section below repeats this warning for the URLs it holds. */}

### Add a new core document

{/* Deliberate exception to the "retarget all of them" note above: this repo is expected to take
over the arbitrum-docs name and URL at cutover, so this one link is left pointing at that eventual
destination rather than at gitConfig's current Fumadocs-test. Do not "fix" this to match gitConfig. */}
...
`content_type` must be exactly one of the seven frontmatter values above, spelled in lowercase, or the build fails. See [Document type conventions](https://github.com/OffchainLabs/Fumadocs-test/blob/main/CONTRIBUTE.md#document-type-conventions) in CONTRIBUTE.md for more on what each type owes the reader.
...
- Many of our docs don't yet follow this guidance. Our team is working on it! If you notice an obvious content bug, feel free to submit an [issue](https://github.com/OffchainLabs/Fumadocs-test/issues) or [PR](https://github.com/OffchainLabs/Fumadocs-test/pulls).

### Banner conventions

{/* The github.com URLs in this section hardcode the owner, repo and branch that gitConfig in lib/shared.ts owns. The Var component cannot sit inside a markdown link destination (CommonMark tokenises the destination before MDX evaluates it), so retarget these by hand if the repo or default branch moves. */}

Use banners to set expectations for your readers or emphasize an important callout, but use them conservatively, since they interrupt the flow of the document. This site renders a banner with the `<VanillaAdmonition>` component; the same callout-syntax rule appears in [STYLE-GUIDE.md](https://github.com/OffchainLabs/Fumadocs-test/blob/main/STYLE-GUIDE.md#what-is-not-here). Docusaurus `:::` directives do not render here and fail the `content:lint` A3 gate, so never write one. The `type` prop is one of `note`, `tip`, `info`, `warning`, or `danger`; pick the type that matches the message, not the page's subject. `title` is optional and defaults to the capitalized type name (`note` becomes "Note", and so on).

**Before writing a new banner, search [`content/partials/CATALOG.md`](https://github.com/OffchainLabs/Fumadocs-test/blob/main/content/partials/CATALOG.md) for one that already says what you need**, and reuse it instead of duplicating the prose. Every catalog entry ends with the directive to paste into your page:

```mdx
<include cwd>content/partials/launch-arbitrum-chain/_raas-providers-notice.mdx</include>
```

The `cwd` flag anchors the path at the repo root, so moving your page later never breaks the include. Inside another partial, drop the flag and write the path relative to the file you are editing. CONTRIBUTE.md's [partial-reuse guide](https://github.com/OffchainLabs/Fumadocs-test/blob/main/CONTRIBUTE.md#reuse-a-partial-before-you-write-new-prose) covers both forms and how to add a partial of your own.
