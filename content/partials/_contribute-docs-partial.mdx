...
{/* Deliberate exception to the docsRepositoryUrl placeholder every other GitHub link in this file
uses: this repo is expected to take over the arbitrum-docs name and URL at cutover, so this one link
is left pointing at that eventual destination rather than at the variable's current value. Do not
"fix" this to match docsRepositoryUrl. */}
