...
1.  Begin by creating a branch (internal) or fork (external) of the [Arbitrum docs repo](https://github.com/OffchainLabs/Fumadocs-test).
2.  Issue a `Draft` pull request into `main`. Pull requests into `main` generate a Vercel preview deployment of your changes; this preview will update as you push commits to your remote.
...
| Tutorial        | Walk a specific reader audience through a comprehensive, guided learning experience |
...
`content_type` must be exactly one of these seven values, or the build fails. See [Document type conventions](https://github.com/OffchainLabs/Fumadocs-test/blob/main/CONTRIBUTE.md#document-type-conventions) in CONTRIBUTE.md for more on what each type owes the reader.
