...
Use banners to set expectations for your readers or emphasize an important callout, but use them conservatively, since they interrupt the flow of the document. This site renders a banner with the `<VanillaAdmonition>` component, documented in [STYLE-GUIDE.md](https://github.com/OffchainLabs/Fumadocs-test/blob/main/STYLE-GUIDE.md#what-is-not-here). Docusaurus `:::` directives do not render here and fail the `content:lint` A3 gate, so never write one. The `type` prop is one of `note`, `tip`, `info`, `warning`, or `danger`; pick the type that matches the message, not the page's subject.
