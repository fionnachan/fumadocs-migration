...
**Routing.** Single locale, no i18n: pages live directly under `content/docs/...` and serve at `/docs/...`. There is no `[lang]` route segment and no locale middleware — `lib/i18n.ts` was deleted 2026-08-18 along with the `ja` and `zh-CN` trees. `proxy.ts` now does exactly three things: (1) **request tracking** for markdown and `llms*.txt` fetches, which runs *before* the bypass list, (2) an explicit **bypass list** of routes served verbatim (`/_next/`, `/img/`, `/favicon.ico`, `/sitemap.xml`, `/robots.txt`, `/llms*`, `/og/`, `/api/`), and (3) `.md`-suffix rewrites plus `Accept: text/markdown` content negotiation to the markdown route. **A new top-level route still belongs in that bypass list** or markdown negotiation will try to rewrite it.

**Request tracking.** `proxy.ts` sends a PostHog `llms_file_fetched` event for `/llms.txt`, `/llms-full.txt`, `/docs/<slug>.md`, `/llms.mdx/docs/<slug>/content.md`, and `Accept: text/markdown` negotiation on a `/docs` URL. All three markdown shapes normalize to the canonical `/docs/<slug>.md`; each request is counted once, because a rewrite does not re-enter the proxy. **Only when `VERCEL_ENV === 'production'`** — nothing fires locally or on preview, and no key is needed to run the app. Key is `NEXT_PUBLIC_POSTHOG_KEY` (same publishable token `lib/posthog.ts` uses). The capture goes through `waitUntil` from `@vercel/functions` and every failure is swallowed and logged, so tracking can never break a response. The IP is never stored; it is hashed with a UTC daily salt into the `distinct_id`. Classification lives in `lib/llms-tracking.ts`, which is **deliberately import-free** so `scripts/lib/llms-tracking.test.mjs` can import it directly under `node --test` (Node 22 strips types natively) and `pnpm test` exercises the real module rather than a copy. Its local copies of `docsRoute`/`docsContentRoute` are pinned to `lib/shared.ts` by two `satisfies` statements in `proxy.ts`, so drift fails `types:check`. Re-adding localization means restoring `defineI18n`, the `i18n` argument to `loader()`, a `[lang]` segment, and `createI18nMiddleware`.
