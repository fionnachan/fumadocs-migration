...
- **Remote images do not work.** Every image in content must be committed under `public/` and referenced as `/img/…`. Markdown images resolve to `next/image` and `next.config.mjs` declares no `images.remotePatterns`, so a remote `https://` src is rejected at render with `hostname … is not configured under images`. `source.config.ts` sets `remarkImageOptions: { external: false }` so the compiler no longer probes remote URLs for their size: before that, one dead third-party URL threw during compilation and 500ed **every** docs page, not just the page holding it. Copy the file into `public/img/` instead, and run `pnpm images:check` to find remote srcs that have rotted. See [INTERNALS](INTERNALS.md#remote-images-are-never-fetched-at-build).
