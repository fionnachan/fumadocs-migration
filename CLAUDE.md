...
- **`drift` finds the upstream checkout itself** via `scripts/lib/upstream-tree.mjs` reading `scripts/data/upstream.config.json`: `--tree-a` (the docs tree, relative to the cwd), then `UPSTREAM_DOCS_REPO` (the repo root, relative to the cwd), then `repo`, then `probePaths` (both relative to **this repo's root**, so they work from a git worktree as well as the main checkout). Clone `arbitrum-docs` beside this repo and `pnpm drift` needs no arguments. **Pairing runs over the whole tree at once** (`pairTrees` in `scripts/lib/tree-compare.mjs`): directory-qualified matches first, each claiming its local file, then the bare-slug fallback for the rest, never onto an already-claimed file. Upstream keeps a concept page and a how-to page under the same basename (`arbos`, `stf`, `batchposter`/`batch-poster`), and per-file resolution let the how-to steal the concept page's local counterpart — three bogus GUTTED ratios hiding three unported how-tos behind them. Do not go back to per-file resolution. Three mechanisms change what the report says and they mean different things: `RENAME_MAP` in `scripts/lib/tree-compare.mjs` pairs a page ported under a new name (so it gets compared — the opposite of suppression); `absentAllowlist` in the config exempts a page deliberately never ported; `guttedAllowlist` exempts a page that **was** ported at parity, where the 70% ratio is only counting Docusaurus `import` lines and inline grid boilerplate. The two allowlists stay separate so an exemption earned for one reason cannot quietly cover the other. Every entry carries its reason, every `guttedAllowlist` entry names its local counterpart, drift lists what it suppressed under `ALLOWED`, and tests pin both lists and assert every `RENAME_MAP` target and `local` path still resolves to a real file — so growing a list is a reviewed act, and an entry that rots into a no-op fails the suite instead of silently regrowing a false positive.
