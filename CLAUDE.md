...
- **`upstream-refresh.yml`** runs Mondays 08:00 UTC (and on `workflow_dispatch`): `nitro:check-release`, then `precompiles:generate`, `contracts:generate` and `cli:generate`, then opens `automated/upstream-refresh` as a PR if anything changed. It installs no Go or Rust toolchain, which is why `cli:generate` parses Nitro's Go source rather than running `nitro --help`. It never writes to `main`, and no-ops when the tree is clean.
...
**Sitemap and robots.** `app/sitemap.ts` and `app/robots.ts` are Next **metadata routes** (file conventions, so no `route.ts` and no hand-written XML; neither sets `revalidate`, which would only restate the default). The sitemap derives one entry per page from `source.getPages()`, plus the home page `/` which is not in the doc collection. There is nothing to exclude: partials, `content/_versions/`, and the glossary all live outside the doc collection `dir`, so upstream's `nonCanonicalRoutePatterns` has no equivalent here. `<lastmod>` is emitted only once `lastModified: true` is set on the docs collection in `source.config.ts` (not enabled yet); the route reads `page.data.lastModified` defensively so turning it on needs no change. `app/robots.ts` emits the non-standard `Content-Signal: search=yes, ai-input=yes, ai-train=no` through a rule's `other` field (Next 16.3.0+). Both read `NEXT_PUBLIC_SITE_URL` with the same `http://localhost:3000` fallback `metadataBase` uses. **That variable is inlined at build time, so an unset value in the Vercel build environment silently ships a production sitemap pointing at localhost**; FS-2668 adds a `getSiteUrl()` helper that throws in production, and both routes should adopt it once it lands. See [INTERNALS](INTERNALS.md#sitemapxml-and-robotstxt).
