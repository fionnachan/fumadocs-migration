...
**Sitemap and robots.** `app/sitemap.ts` and `app/robots.ts` are Next **metadata routes** (file conventions — no `route.ts`, no hand-written XML). The sitemap derives one entry per page from `source.getPages()`, plus the home page `/` which is not in the doc collection. There is nothing to exclude: partials, `content/_versions/`, and the glossary all live outside the doc collection `dir`, so upstream's `nonCanonicalRoutePatterns` has no equivalent here. `<lastmod>` is emitted only once `lastModified: true` is set on the docs collection in `source.config.ts` (not enabled yet); the route reads `page.data.lastModified` defensively so turning it on needs no change. `app/robots.ts` emits the non-standard `Content-Signal: search=yes, ai-input=yes, ai-train=no` through a rule's `other` field (Next 16.3.0+). Both read `NEXT_PUBLIC_SITE_URL` with the same `http://localhost:3000` fallback `metadataBase` uses. See [INTERNALS](INTERNALS.md#sitemapxml-and-robotstxt).
