...
**Page metadata.** `generateMetadata` in the docs page emits title, description, the OG image from the `og/` route, `alternates.canonical`, and `twitter` (`summary_large_image`, `@arbitrum`). URLs are written **relative** and resolved against `metadataBase` in `app/layout.tsx` (`NEXT_PUBLIC_SITE_URL`, falling back to `http://localhost:3000`). Unset that variable in Vercel and every canonical and social image URL in production points at localhost. The canonical uses `page.url`, which has no query string, so `?v=` archived views canonicalize to the live page. `app/(home)/page.tsx` has no canonical.
